...
public class ReservationRepository implements IReservationRepository {
	...
	public boolean reserveRoom(User u, Room r, LocalDateTime start, LocalDateTime stop) {
		...
		if(isRoomFree(r,start) && isRoomFree(r,stop)){
			...
			System.out.println("Użytkownik "+u.getName()+" zarejestrował pokój "+r.name+" od "+start+" do "+stop );
			return true;
		}else{
			Reservation res=getReservation(r,stop);
			if(u==res.owner){
				updateReservation(res,stop);
				System.out.println("Użyszkodnik "+u.getName()+" potwierdził rezerwację pokoju "+r.name+" od " + res.timeStart + " do " + res.timeStop);
				return true;
			}
			System.out.println("Użytkownik "+u.getName()+ " nie może zarezerwować sali "+ r.name + " gdyż aktualnie zajął ją "+res.owner.getName());
			return false;
		}
	}
	...
	@Override
	public Room getFreeRoomInGroup(Group group, LocalDateTime timestamp ){ //szukamy wolnego pokoju w grupie i jej podgrupach

		Room result=null;
		for(Room el:group.rooms){ //szukamy w pokojach
	...
	public Room getNearestFreeRoom(Room startRoom, LocalDateTime timestamp ){
		...
		while(result==null){
			if(currentMain.leftGroup==null && currentMain.rightGroup==null && currentMain.parent==null){
				return null; //jesteśmy na górze i nic
			}else if(currentLeft==null && currentRight==null){ //przeszliśmy cały poziom i wspinamy się do góry
	...
	public DeviceMessage getRoomStatus(Room room , LocalDateTime timestamp ){
		...
		if(oldIdStatus!=result.idStatus){
			...
			case "1_2": result.updateDate=timestamp.plusSeconds(15L); 
			break;
			case "2_3": result.updateDate=res.timeStop;
			break;
			case "2_1":
			case "3_1":	result.updateDate=getNextReservation(room, timestamp);
			break;		
